...
if (isPost('loadAllTemplates')) {
  ...
  if ($templateId) {
    ...
    $stmt2 = $conn->prepare("
            SELECT session_id, session_day, session_of_the_day, module_code,
                   session_code, session_mode, delivery_mode, location, session_name, topics, hours, session_type, faculty
            FROM template_data
            WHERE template_id=?
  ...
  if (empty($modulesOrdered) && !empty($grouped)) {
    $modulesOrdered = array_keys($grouped);
  }
  ...
  $_SESSION['selected']['template_id']   = $templateId;
...
<body class="py-4">
  ...
  <script>
    ...
    searchInput?.addEventListener('input', debounce(async () => {
      ...
      const res = await fetch('/schedule_gen/admin/course/search_templates.php?q=' + encodeURIComponent(q));
